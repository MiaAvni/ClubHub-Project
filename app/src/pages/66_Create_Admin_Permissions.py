import streamlit as st
import requests
from streamlit_extras.app_logo import add_logo
from modules.nav import SideBarLinks

# Initialize sidebar
SideBarLinks()

st.title("Create Admin Permissions")

# Initialize session state for modal
if "show_success_modal" not in st.session_state:
    st.session_state.show_success_modal = False
if "success_admin_id" not in st.session_state:
    st.session_state.success_admin_id = ""
if "reset_form" not in st.session_state:
    st.session_state.reset_form = False
if "form_key_counter" not in st.session_state:
    st.session_state.form_key_counter = 0

# Define the success dialog function
@st.dialog("Success")
def show_success_dialog(admin_id):
    st.markdown(f"### Admin permissions for Admin ID {admin_id} have been successfully created!")
    
    # Create two buttons side by side
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("Return to Admin Directory", use_container_width=True):
            st.session_state.show_success_modal = False
            st.session_state.success_admin_id = ""
            st.switch_page("pages/60_administrator_home.py.py")
    
    with col2:
        if st.button("Add Another Permission", use_container_width=True):
            st.session_state.show_success_modal = False
            st.session_state.success_admin_id = ""
            st.session_state.reset_form = True
            st.rerun()

# Handle form reset
if st.session_state.reset_form:
    st.session_state.form_key_counter += 1
    st.session_state.reset_form = False

# API endpoint
API_URL = "http://web-api:4000/adminPermissions"

# Create a form for admin permissions with dynamic key to force reset
with st.form(f"add_admin_permissions_form_{st.session_state.form_key_counter}"):
    st.subheader("Admin Permission Information")

    # Required fields
    admin_id = st.number_input("Admin ID *", min_value=1, step=1)
    permission = st.text_input("Permission *")

    # Form submission button
    submitted = st.form_submit_button("Create Permission")

    if submitted:
        # Validate required fields
        if not all([admin_id, permission]):
            st.error("Please fill in all required fields marked with *")
        else:
            # Prepare the data for API
            permission_data = {
                "adminID": int(admin_id),
                "permission": permission,
            }

            try:
                # Send POST request to API
                response = requests.post(API_URL, json=permission_data)

                if response.status_code == 201:
                    # Store admin ID and show modal
                    st.session_state.show_success_modal = True
                    st.session_state.success_admin_id = admin_id
                    st.rerun()
                else:
                    st.error(
                        f"Failed to create permission: {response.json().get('error', 'Unknown error')}"
                    )

            except requests.exceptions.RequestException as e:
                st.error(f"Error connecting to the API: {str(e)}")
                st.info("Please ensure the API server is running")

# Show success modal if permission was created successfully
if st.session_state.show_success_modal:
    show_success_dialog(st.session_state.success_admin_id)

# Add a button to return to the Admin Directory
if st.button("Return to Admin Perimssions Directory"):
    st.switch_page("pages/61_admin_permissions_directory.py")